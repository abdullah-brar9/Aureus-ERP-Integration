...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('accounting_coa_import_batches', function (Blueprint $table) {
            $table->string('source_sheet')->nullable()->after('filename');
            $table->unsignedInteger('header_row_number')->nullable()->after('source_sheet');
            $table->char('file_hash', 64)->nullable()->after('header_row_number');
            $table->json('original_headers')->nullable()->after('file_hash');
            $table->json('metadata_rows')->nullable()->after('original_headers');
        });
        ...
        Schema::table('accounts_bank_statements', function (Blueprint $table) {
            ...
            $table->string('account_title')->nullable()->after('bank_account_number');
            $table->date('statement_start_date')->nullable()->after('date');
            $table->date('statement_end_date')->nullable()->after('statement_start_date');
            $table->decimal('opening_balance', 20, 4)->default(0)->after('statement_end_date');
            $table->decimal('total_debits', 20, 4)->default(0)->after('opening_balance');
            $table->decimal('total_credits', 20, 4)->default(0)->after('total_debits');
            $table->decimal('closing_balance', 20, 4)->default(0)->after('total_credits');
            $table->string('original_filename')->nullable()->after('closing_balance');
            $table->char('file_hash', 64)->nullable()->after('original_filename');
            $table->string('source_sheet')->nullable()->after('file_hash');
            $table->string('parser')->nullable()->after('source_sheet');
            $table->string('import_status')->default('awaiting_review')->after('parser');
            $table->json('validation_errors')->nullable()->after('import_status');
            $table->json('raw_header')->nullable()->after('validation_errors');

            $table->unique(['company_id', 'file_hash'], 'bank_statements_company_file_hash_unique');
            $table->index(['company_id', 'bank_account_number', 'statement_start_date'], 'bank_statements_account_period_index');
        });

        Schema::table('accounts_bank_statement_lines', function (Blueprint $table) {
            $table->date('transaction_date')->nullable()->after('internal_index');
            ...
            $table->decimal('debit', 20, 4)->default(0)->after('reference');
            $table->decimal('credit', 20, 4)->default(0)->after('debit');
            $table->decimal('running_balance', 20, 4)->nullable()->after('credit');
            $table->unsignedInteger('source_row')->nullable()->after('running_balance');
            $table->json('raw_row')->nullable()->after('source_row');
            $table->char('transaction_fingerprint', 64)->nullable()->after('raw_row');
            $table->string('import_status')->default('imported')->after('transaction_fingerprint');

            $table->unique(['statement_id', 'transaction_fingerprint'], 'bank_statement_lines_fingerprint_unique');
